import React, { ComponentProps } from 'react'
import { twMerge } from 'tailwind-merge'

type InputPrefixProps = ComponentProps<'div'>

export const InputPrefix = (props: InputPrefixProps) => {
  return <div {...props} />
}

type InputControllerProps = ComponentProps<'input'>

export const InputController = (props: InputControllerProps) => {
  return (
    <input
      type="text"
      className="pl flex-1 border-0 bg-transparent p-0 text-zinc-900 placeholder-zinc-600 outline-none dark:text-zinc-100 dark:placeholder:text-zinc-400"
      {...props}
    />
  )
}

export type InputRootProps = ComponentProps<'div'>

export const InputRoot = (props: InputRootProps) => {
  return (
    <div
      className={twMerge(
        ' flex w-full items-center gap-2 rounded-lg border border-zinc-300 bg-none px-3 py-2 shadow-sm',
        'focus-within:border-violet-500 focus-within:ring-4 focus-within:ring-violet-100',
        'dark:border-zinc-700 dark:bg-zinc-800 dark:focus-within:border-violet-600 dark:focus-within:ring-violet-500/20',
        props.className,
      )}
      {...props}
    ></div>
  )
}
